...
tags: [area/desarrollo, comp/backend, comp/crm, stack/fastapi, stack/postgres, estado/pendiente]
---
...
- [ ] **T02.1.1** — Modelos SQLAlchemy: tenant, lead, conversacion, mensaje, asesor, evento.
...
- [ ] **T02.1.2** — Generar migración inicial con Alembic.
  - **Criterio:** `alembic upgrade head` crea todas las tablas.

### Etapa 2.2 — Schemas y repositorios
- [ ] **T02.2.1** — Schemas Pydantic (in/out) para lead, mensaje, evento.
...
- [ ] **T02.2.2** — Capa de repositorio/servicio para CRUD de leads y emisión de eventos.
...
### Etapa 2.3 — API REST
- [ ] **T02.3.1** — Router de leads: crear, listar (con filtros), detalle, actualizar estado.
...
- [ ] **T02.3.2** — Router de mensajes/conversación de un lead.
  - **Criterio:** `GET /leads/{id}/mensajes` y `POST /leads/{id}/mensajes`.
- [ ] **T02.3.3** — Router de métricas (lee de `eventos`/`leads`).
