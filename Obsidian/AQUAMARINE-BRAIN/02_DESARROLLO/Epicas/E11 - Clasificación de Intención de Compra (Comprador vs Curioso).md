...
tags: [area/desarrollo, comp/agente, comp/crm, comp/dashboard, stack/claude, estado/completado]
---
...
> [!success] Estado: COMPLETADA + AUDITADA (2026-07-27) · post-MVP
> Épica **construida y auditada** (commit `ba66bb7`, rama `feat/e11-intencion`): **261/261 tests verdes**, `npm run build` OK. Decisión de origen: [[Decisiones (Decision Log)]] **D26**. Feature doc: `intencion.md` (raíz). Handoffs: [[🗺️ MOC - Prompts (Handoffs)]] → E11 (1 motor · 2 métrica+surface · 3 tests+docs · 4 fix de `CLAUDE.md`). Pendiente: **merge a `master`**.
>
> **Reconciliaciones de la auditoría** (el código se aparta del texto literal de 2 casillas, y hace bien — NO cambiar el código):
> 1. **T11.1.1 — `intencion` NO se agrega a `_CAMPOS_PERFIL`** (solo `interes_urgencia`). Se persiste vía `lead_service.registrar_intencion` (escritor único) para que el evento `intencion_clasificada` se emita **solo al cambiar** (T11.3.1); si `fusionar_perfil` la pre-escribiera, el evento nunca dispararía. El resultado neto de T11.1.1 (intención persistida cada turno) **sí se cumple**.
> 2. **Principio 2 vs T11.4.1** — el desglose `por_zona` **sí lee Chroma** para enumerar las zonas del inventario (reusa la lógica del heatmap, como pide T11.4.1); de Postgres viene la **clave de clasificación del lead** (`perfil.zona`). El "no Chroma" del principio 2 aplica a la clasificación del lead, no al universo de zonas. Sin contradicción real.
...
- [x] **T11.1.1** — Extender el profiler con `intencion` + persistir `interes_urgencia`.
  ...
  - **Criterio:**
    - [x] `PerfilExtraido` gana `intencion: Literal["comprador","explorando","curioso"] | None` y su tool schema lo incluye con una **descripción que enseñe la rúbrica** (D26).
    - [x] `_CAMPOS_PERFIL` incluye **`intencion`** e **`interes_urgencia`** → ambos se persisten en `lead.perfil` (patrón idéntico a `plazo`/`movilidad`); `fusionar_perfil` no pisa con `None`.
    - [x] El system prompt de extracción (`_EXTRACTION_SYSTEM`) explica cómo mapear señales → intención (precio-solo/sin-presupuesto/"solo mirando" → curioso; presupuesto+plazo/visita/asesor → comprador; intermedio → explorando).
    - [x] Tests con `PerfilExtraido` mockeado: un chat "¿cuánto vale?" sin más → `curioso`; uno con presupuesto+plazo corto → `comprador`.
...
- [x] **T11.2.1** — `derivar_intencion(lead|perfil, temperatura)` pura (fallback determinista).
  ...
  - **Criterio:**
    - [x] `derivar_intencion(perfil, temperatura) -> "comprador"|"explorando"|"curioso"` sin red ni SDK: `comprador` si `temperatura in {caliente}` o (`presupuesto` y `plazo in {corto,medio}`) o `inmueble_interes` presente; `curioso` si `frio` y (sin presupuesto y `plazo=="largo"`); `explorando` en el resto.
    - [x] Determinista y alineada con la rúbrica de D26; tests de casos límite.
...
- [x] **T11.2.2** — Script `backfill_intencion.py` idempotente.
  ...
  - **Criterio:**
    - [x] Recorre `Lead` del tenant; si `perfil` no tiene `intencion`, la deriva con `derivar_intencion` y la guarda (merge de `perfil`, sin pisar otras claves). Idempotente; flags `--dry-run`/`--tenant`; imprime stats `{total, ya_tenian, derivados, por_intencion}`.
    - [x] Emite el evento de auditoría (T11.3.1) por cada lead derivado.
...
- [x] **T11.3.1** — Emitir `intencion_clasificada` en `lead_service` cuando la intención se asigna/cambia.
  ...
  - **Criterio:**
    - [x] Nuevo `tipo` de evento **`intencion_clasificada`** con `payload = {"intencion", "zona", "ciudad", "inmueble_interes"}`.
    - [x] Se emite **solo cuando cambia** (evita ruido por turno); el evento lo emite `lead_service`, **no** el router (respeta el patrón: solo `lead_service`/`handoff` emiten eventos).
    - [x] Tests: cambio de intención emite 1 evento; sin cambio no emite.
...
- [x] **T11.4.1** — Endpoint `GET /metrics/intencion` (al vuelo, tolerante, tenant-scoped).
  ...
  - **Criterio:**
    - [x] Carga `Lead` del tenant (patrón `metrics.py`); para cada lead usa `perfil['intencion']` **o**, si falta, `derivar_intencion(perfil, temperatura)` como fallback (para no depender del backfill).
    - [x] `por_intencion` global con `Rate {pct,num,den}`; `por_zona: [{zona, comprador, explorando, curioso, pct_comprador, pct_curioso}]` usando `_cumple_ubicacion`/`leads_por_ubicacion` (match tolerante, consistente con el heatmap); `por_propiedad` agrupando por `perfil['inmueble_interes']`.
    - [x] Ordena zonas/propiedades por volumen; scoped por `tenant_id`; tests con leads mockeados (incluye un curioso "cerca del metro" que no matchea zona real → cuenta en global, no en zona).
...
- [x] **T11.5.1** — Tool de insights `intencion_por_zona` (edit de 3 pasos).
  ...
  - **Criterio:**
    - [x] Añadir la tool a `TOOLS` (con param opcional `zona`), un `ejecutar_intencion(db, tenant, zona=None)` que reusa la métrica de T11.4.1, y una rama en el dispatcher `ejecutar_tool`. Sin tocar `insights_agent.py` ni `api/insights.py`.
...
- [x] **T11.5.2** — Tile "Compradores vs Curiosos" en el dashboard.
  ...
  - **Criterio:**
    - [x] Tile con el % comprador vs curioso (barra/donut) + top zonas por % de curiosos; consume `GET /metrics/intencion`; respeta paleta de marca y `tsc` estricto (`npm run build` verde).
...
- [x] **T11.6.1** — Suite de tests del feature.
...
- [x] **T11.6.2** — `intencion.md` + `CLAUDE.md` + `Modelo de Datos.md`.
  ...
  - **Criterio:**
    - [x] `intencion.md`: qué resuelve, la rúbrica, cómo se infiere/deriva, el evento, la métrica segmentada, cómo correr el backfill y los tests.
    - [x] Fila nueva en la tabla "Documentación por feature" de `CLAUDE.md`.
    - [x] **Para el Planner (no lo toca el Dev):** en `Modelo de Datos.md` documentar `perfil.intencion` + `perfil.interes_urgencia`, el `eventos.tipo = intencion_clasificada`, y la métrica derivada; citar D26; bump `actualizado`.
