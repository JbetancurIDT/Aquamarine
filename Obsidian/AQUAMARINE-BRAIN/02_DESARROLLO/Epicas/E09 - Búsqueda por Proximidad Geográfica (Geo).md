...
tags: [area/desarrollo, comp/rag, comp/agente, stack/chroma, stack/claude, estado/completado]
---
...
> **CORE (S1–S6) + STRETCH (S7–S9)** entregados, más tres pulidos plegados aquí: **S10** (H7 lugares tolerantes), **S11** (H8 cercanía nombrada + foco) y **S12** (preferencia de movilidad). Todo mergeado a `master`; **~228 tests backend en verde**. Decisiones asociadas: [[Decisiones (Decision Log)]] **D21** (alcance), **D24** (datos vivos), **D25** (movilidad SUAVE). La **visualización en mapa** vive en épica aparte → **[[E10 - Mapa de Inmuebles]]** (D22/D23). **Pendiente menor:** verificación visual (eyeball) del usuario. Detalle técnico en `geo.md`.
...
- [x] **T09.1.1** — `geo_const.py`: vocabulario congelado de categorías y claves.
  ...
  - **Criterio:**
    - [x] `CERCANIA_KEYS` con los **7 slugs congelados** → clave: `metro`→`dist_metro_m`, `supermercado`→`dist_super_m`, `centro_comercial`→`dist_mall_m`, `colegio`→`dist_colegio_m`, `universidad`→`dist_universidad_m`, `parque`→`dist_parque_m`, `clinica`→`dist_clinica_m`.
    - [x] `ETIQUETA_CAT` con la frase legible por categoría ("una estación de metro", "un supermercado", …).
    - [x] `clave_geocache(zona, ciudad)` → clave normalizada `"{zona}|{ciudad}"` (minúsculas, sin acentos, misma regla que `_norm` de `search.py`). **Sin** sufijo `antioquia|co`.
    - [x] Se decide y documenta que **todos** los JSON/coords usan `lat`/`lon` (no `lng`), y el nombre exacto de cada archivo de datos.
...
- [x] **T09.2.1** — Semilla de estaciones de metro + centroides de zona (sin red).
...
- [x] **T09.3.1** — Extender `InmuebleIn` con los 7 `dist_<cat>_m`.
  ...
  - **Criterio:**
    - [x] 7 campos `int | None = None` bajo el bloque `# --- Geo ---` (usar los nombres de `CERCANIA_KEYS`).
    - [x] Las 7 claves se añaden al dict `bruto` de la property `metadata`; la comprensión final que filtra `None` **no se toca** → si una distancia es `None`, la clave se omite.
    - [x] `.document` (texto embebido) **no cambia** (las distancias son señal numérica de filtro, no semántica).
...
- [x] **T09.3.2** — `geo.py`: haversine + distancias por categoría (funciones puras).
  ...
  - **Criterio:**
    - [x] `haversine_m(lat1,lon1,lat2,lon2) -> float` (metros, radio 6_371_000); acierta ±0.5% contra una distancia conocida del Valle de Aburrá.
    - [x] `dist_poi_mas_cercano_m(lat,lon,pois) -> int | None` (None si lat/lon None **o** lista vacía; si no, `round(min(...))`).
    - [x] `distancias_por_categoria(lat,lon,pois_por_cat) -> dict[str,int]` que **nunca** incluye una clave con valor `None`.
    - [x] Lectores `cargar_metro()`, `cargar_centroides()` desde `app/rag/data/`; reusa `clave_geocache` de `geo_const`.
...
- [x] **T09.3.3** — `seed_geo.py`: backfill idempotente de coords + distancias.
  ...
  - **Criterio:**
    - [x] `col.get(where={"tenant_id":{"$eq":tenant}}, include=["metadatas"])`; por inmueble **copia** su metadata (no la reconstruye desde cero → no pierde `titulo/precio/imagenes`).
    - [x] Rellena `latitud/longitud` desde `_COORDS_ZONA` (con jitter determinista ±0.002° por `inmueble_id`) **solo si** faltan o son sintéticas (detecta el par `(6.000123,-75.000456)` y `abs(lat)<0.01 or abs(lon)<0.01`).
    - [x] Calcula `dist_<cat>_m` con `geo.distancias_por_categoria` (estaciones de metro + POIs semilla). **Municipios sin metro no reciben `dist_metro_m`** (honestidad by design).
    - [x] Reconstruye el dict completo (metadata previa − `dist_*` obsoletas + nuevas) y hace `col.update(ids, metadatas)` por chunks. **Verifica primero si la versión instalada de chromadb hace merge o replace** en `update` y ajústate para borrado determinista de claves.
    - [x] **Idempotente:** dos corridas seguidas dejan idéntica metadata. Sin red. Flags `--dry-run`, `--tenant`. Imprime stats `{total, geocodificados, ya_tenian_coords, sin_coords, con_alguna_dist}`.
...
- [x] **T09.3.4** — Pin de `chromadb` en `requirements.txt`.
...
- [x] **T09.4.1** — `_cercania_cond` + `_radio_m` en `search.py`.
  ...
  - **Criterio:**
    - [x] `buscar_inmuebles(query, {"cerca_de":"metro"})` inyecta `{"dist_metro_m":{"$lte":radio_m}}` ANDeado con `tenant_id` en `_where_duro`.
    - [x] Inmuebles **sin** la clave (Rionegro/sin-coords) **no matchean** → honestidad gratis por la semántica de `$lte`.
    - [x] `radio_km` del lead sobrescribe el default por categoría; **radio efectivo nunca < 1500 m** (piso honesto por el error de centroide de barrio; ver Riesgos).
    - [x] Firma pública de `buscar_inmuebles` **sin cambios** (`cerca_de`/`radio_km` viajan dentro de `filtros`).
    - [x] **Sin "Nivel 4 / radio ampliado ×2":** para un filtro espacial duro, **vacío + mensaje honesto** es la respuesta correcta (no se ensancha el radio en silencio).
...
- [x] **T09.5.1** — Tool `cerca_de`/`radio_km` + distancia en el texto.
  ...
  - **Criterio:**
    - [x] `input_schema.filtros` gana `cerca_de` (enum de 7 slugs de `CERCANIA_KEYS`) y `radio_km` (number), con descripciones que enseñen el mapeo de frases ("estación/tranvía/metrocable"→metro; "D1/Ara/Éxito/Carulla/Jumbo/mercado"→supermercado; "mall/C.C."→centro_comercial; "clínica/hospital/EPS"→clinica).
    - [x] `_frase_cercania(inm, cat)` lee `dist_<cat>_m` y formatea "[A ~600 m de … — aprox.]" (m si <1000, km si ≥1000); `_formatear_linea` la concatena.
    - [x] Encabezado **honesto** cuando se pidió `cerca_de` y la tool vino **vacía**: instruye a Aqua a NO afirmar "no existe nada" y a ofrecer ampliar distancia / cambiar de zona / quitar el requisito (recordando que el metro solo cubre el Valle de Aburrá).
...
- [x] **T09.5.2** — System prompt: vocabulario, honestidad y distancia aproximada.
  ...
  - **Criterio:**
    - [x] Nueva sección "## Búsqueda por cercanía" con las 7 categorías y sus disparadores (incl. cadenas de súper).
    - [x] Regla de distancia **aproximada**: "a **unos ~600 m** de la estación", "a **pocos minutos** de un Éxito"; **prohibido** cifras exactas ("a 340 m"), "caminando" o "a X cuadras".
    - [x] Honestidad geográfica **dura**: el **metro solo existe en el Valle de Aburrá**; si piden "cerca del metro" en Rionegro/La Ceja/El Retiro/Guatapé/Cartagena/Coveñas → decirlo con calidez, no inventar.
    - [x] Regla: `cerca_de` + tool vacía ≠ "no existe"; ofrecer alternativas.
    - [x] Sin fechas ni IDs (no invalida el prompt caching). Los tests de contenido del prompt (si existen) siguen verdes.
...
- [x] **T09.6.1** — Tests offline (Chroma mockeado, sin red ni SDK).
  ...
  - **Criterio:**
    - [x] `haversine_m` determinista ±0.5% contra una distancia conocida; simétrica; punto consigo mismo = 0.
    - [x] `distancias_por_categoria` **no** emite claves para categorías vacías ni con lat/lon None.
    - [x] `InmuebleIn.metadata` omite `dist_*` None y emite los presentes como `int`; `.document` no cambia (regresión).
    - [x] `cerca_de="metro"` produce el `$lte` correcto en el `where`; inmueble **sin** `dist_metro_m` no aparece; `radio_km` sobrescribe default y respeta el piso 1500 m.
    - [x] `test_categoria_sin_metro_es_honesto`: inmuebles del oriente (sin `dist_metro_m`) no se devuelven como "cerca del metro"; el texto habilita la respuesta honesta.
    - [x] Backfill (`seed_geo`) con `col.get` mockeado (uno con coords, uno sin): el primero recibe `dist_*`, el segundo no; segunda corrida idempotente; no pierde `titulo/precio/imagenes`.
...
- [x] **T09.6.2** — `geo.md` + fila en `CLAUDE.md`.
...
- [x] **T09.7.1** — `build_metro.py`: estaciones desde el GTFS oficial.
...
- [x] **T09.7.2** — `build_poi.py`: POI del Valle de Aburrá vía Overpass.
...
- [x] **T09.7.3** — `build_geocache.py`: geocodificar pares (zona,ciudad) con Nominatim.
...
- [x] **T09.8.1** — Config + `geocode_vivo` (Nominatim + caché persistente + rate-limit).
...
- [x] **T09.8.2** — `buscar_por_lugar` + routing `cerca_de_lugar` en la tool.
  ...
  - **Criterio:**
    - [x] `buscar_por_lugar(nombre, filtros=None, k=3, radio_km=None)` devuelve dict `{estado, punto, resultados, descartados_sin_coords}` con `estado ∈ {ok, lugar_no_encontrado, sin_coords}`; usa `col.get` + `_where_duro` (respeta precio/hab); excluye sin-coords y los cuenta; ordena ascendente; marca `coincidencia="cercana"` + `motivo="a ~X km de <lugar>"`.
    - [x] Tool: `filtros.cerca_de_lugar` (string), **excluyente** con `cerca_de`. Routing en `ejecutar_buscar_inmuebles`: `codigo` > `cerca_de_lugar` > `cerca_de` > `query/filtros`.
    - [x] Cada `estado` produce texto honesto (lugar no ubicado → pedir referencia alterna, no negar inventario).
    - [x] Prompt: cuándo usar `cerca_de_lugar` (nombre propio) vs `cerca_de` (categoría).
...
- [x] **T09.8.3** — Tests del fallback + conversaciones demo.
  ...
  - **Criterio:**
    - [x] Tests (geocode y Chroma `col.get` mockeados): rankea por cercanía; `lugar_no_encontrado`; inmueble sin coords excluido y contado; todo-sin-coords; respeta `precio_max`; `radio_km` recorta; handler produce texto honesto.
    - [x] `seed_demo.py`: 2 leads que ejerciten cercanía (uno "cerca del metro"/D1 en el Valle de Aburrá con inmueble real p. ej. `9907677`; otro "cerca de EAFIT"/"Clínica Las Américas" con `9718612`) + un intercambio de honestidad "en esa zona no hay metro" (Guatapé). Revisar que `_calcular_esperados` siga cuadrando.
...
- [x] **T09.9.1** — Enriquecimiento falla-suave en `ingest.py` (fichas nuevas nacen con geo).
...
### Sprint 10 — Lugares tolerantes [POST-STRETCH · H7]
El lead nombra lugares en **coloquial** ("la piedra del peñol", "la 70") y a veces **ambiguos** ("La América"). Aqua traduce a lo oficial y desambigua antes de geocodificar.

- [x] **T09.10.1** — Geocoder robusto + desambiguación.
  - **Archivos:** `backend/app/rag/geo.py`, `backend/app/agent/prompts.py`, `backend/app/agent/tools.py`.
  - **Criterio:**
    - [x] El prompt traduce coloquial→oficial, cualifica municipio/departamento y **desambigua** (usa el contexto o **pregunta**) antes de geocodificar.
    - [x] `geocode_vivo` endurecido: `_con_region`/`_simplificar`/`_consulta_nominatim` (limit=5 por `importance` + fallback simplificado), rate-limit **thread-safe**; `_simplificar` no quita artículos de topónimo (La Ceja).
    - [x] Verificado live: "piedra del peñol" → "Mirador del Peñol, El Peñol, Antioquia"; "La América" sin contexto → **pregunta**; con "sector Estadio" → resuelve.

### Sprint 11 — Cercanía nombrada y foco [POST-STRETCH · H8]
Dos comportamientos: responder "¿qué hay cerca?" con **nombres reales** de POIs, y no re-listar cuando el lead se **enfoca** en una propiedad.

- [x] **T09.11.1** — Tool `lugares_cerca` + foco en una propiedad.
  - **Archivos:** `backend/app/rag/geo.py`, `backend/app/agent/tools.py`, `backend/app/agent/orchestrator.py`, `backend/app/agent/prompts.py`.
  - **Criterio:**
    - [x] `geo.lugares_cerca` devuelve nombres reales por categoría (omite vacías, **dedup por nombre**); tool `lugares_cerca` responde texto sin tarjetas.
    - [x] El orquestador despacha por registro `{buscar_inmuebles, lugares_cerca}`; el prompt hace **seguimiento de UNA propiedad** (no re-corre búsqueda general).
    - [x] Verificado live: foco → 1 ficha (no 3); "¿qué hay cerca?" → EAFIT ~1.3 km, Carulla ~200 m, Mall Zona Dos…

### Sprint 12 — Preferencia de movilidad [POST-STRETCH · D25]
La forma de moverse del lead reordena resultados; **nunca** filtra (contrasta con la cercanía DURA de D21).

- [x] **T09.12.1** — Re-ranking SUAVE por movilidad.
  - **Archivos:** `backend/app/agent/profiler.py`, `backend/app/agent/prompts.py`, `backend/app/rag/search.py`, `backend/app/agent/tools.py`.
  - **Criterio:**
    - [x] `profiler` capta `perfil.movilidad` (no califica); Aqua pregunta **UNA vez** antes de la 1ª búsqueda general y mapea carro→parqueadero, metro→cerca_metro, a-pie/bici→conectado, desde-casa→espacio_oficina.
    - [x] `buscar_inmuebles(query, filtros, k, preferencias)` reordena por nº de preferencias cumplidas (estable, marca `preferencias_ok`) y **nunca excluye**.
    - [x] Verificado live: metro reordena sin excluir; Aqua pregunta y mapea la movilidad una sola vez.
...
Los tests backend suben a **~228 en verde** (145 del MVP + geo CORE + fallback por nombre propio + mapa + movilidad); todos sin gastar APIs. **Los 4 escenarios se verificaron en vivo** con key real (metro honesto, D1, Guatapé sin metro, EAFIT por nombre propio).
