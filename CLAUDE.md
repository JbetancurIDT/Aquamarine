...
| Intención de compra (E11) | [intencion.md](intencion.md) | Eje `perfil.intencion ∈ {comprador, explorando, curioso}` inferido por Aqua + `derivar_intencion` de fallback + evento `intencion_clasificada` + backfill + **`GET /metrics/intencion`** (% por zona/propiedad, al vuelo) + tool insights `intencion_por_zona` + tile dashboard. **Completo (handoffs 1–3, auditado)** · 261 tests verdes. Eje independiente: NO toca `scoring.py` (D26) |
